...
export default function MeasurementDetails({ userId }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
      {loading && (
        <div className="absolute top-0 w-full h-1 bg-indigo-500 animate-pulse z-20" />
      )}
      ...
      <div className="p-6 space-y-6">
        ...
        <section className="mt-4 p-4 border-2 border-dashed border-gray-100 rounded-lg bg-gray-50/30">
          <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">
            <ImageIcon className="w-3 h-3 text-indigo-600" /> Design Reference
            Link
          </div>
            ...
            {fieldsByDress[dressType]?.map((field) => (
              <div key={field}>
                ...
                <input
                  ...
                  className="w-full h-9 px-2 rounded border border-gray-200 text-sm focus:border-indigo-500 outline-none font-mono"
                />
              </div>
            ))}
          </div>
        ...
        <button
          onClick={handleSave}
          disabled={loading}
          className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg font-bold text-white transition-all ${loading ? "bg-gray-400 cursor-not-allowed" : success ? "bg-green-600" : "bg-indigo-600 hover:bg-indigo-700 shadow-md"}`}
        >
          {loading ? (
            "Processing..."
          ) : success ? (
