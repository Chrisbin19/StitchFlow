...
import { Settings, LogOut, Scissors, Sun, Moon } from "lucide-react";
import Link from "next/link";
import CutterNotificationBell from "./CutterNotificationBell";

// Global CSS token system injected into the DOM
const CutterThemeVars = () => (
  <style>{`
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        total++;
        const d = doc.data();
        if (d.status === "Completed" || d.status === "STITCHING_COMPLETED" || d.status === "CUTTING_COMPLETE") {
          const t = d.updatedAt?.seconds * 1000 || 0;
          if (t >= startOfToday.getTime()) completed++;
        }
      });
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-40 w-full adm-font-body"
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
              ...
              <span className="adm-font-display font-black text-white text-sm tracking-wide">SK</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <CutterNotificationBell
              onSelectTask={t => console.log(t)}
            />
        ...
        <div style={{ width: '100%', height: '2px', background: 'transparent', position: 'absolute', bottom: -1 }}>
          <div style={{ width: `${pct}%`, height: '100%', background: 'var(--adm-emerald)', transition: 'width 1s ease-out' }} />
        </div>
      </header>
    </>
  );
